...
contract Exchange is Owned {
  ...
  enum OrderSelfTradePrevention {DecreaseAndCancel, CancelOldest, CancelNewest, CancelBoth}
  ...
  event WalletExitWithdrawn(address indexed wallet, address asset, uint256 quantity);
  ...
  function setChainPropagationDelay(uint256 _chainPropagationDelay) external onlyAdmin {
    require(_chainPropagationDelay < maxChainPropagationDelay, 'Must be less than 1 week');
    emit ChainPropagationDelayChanged(chainPropagationDelay, _chainPropagationDelay);
  ...
  function setFeeWallet(address _feeWallet) external onlyAdmin {
    ...
    require(_feeWallet != feeWallet, 'Must be different from current fee wallet');
    emit FeeWalletChanged(feeWallet, _feeWallet);
    feeWallet = _feeWallet;
  }

  function setWithdrawalFeeBasisPoints(uint64 _withdrawalFeeBasisPoints) external onlyAdmin {
    require(_withdrawalFeeBasisPoints < maxWithdrawalFeeBasisPoints, 'Excessive withdrawal fee');
    emit WithdrawalFeeChanged(withdrawalFeeBasisPoints, _withdrawalFeeBasisPoints);
    withdrawalFeeBasisPoints = _withdrawalFeeBasisPoints;
  }

  function setTradeMakerFeeBasisPoints(uint64 _tradeMakerFeeBasisPoints) external onlyAdmin {
    require(_tradeMakerFeeBasisPoints < maxTradeFeeBasisPoints, 'Excessive maker fee');
    emit TradeMakerFeeChanged(tradeMakerFeeBasisPoints, _tradeMakerFeeBasisPoints);
    tradeMakerFeeBasisPoints = _tradeMakerFeeBasisPoints;
  }

  function setTradeTakerFeeBasisPoints(uint64 _tradeTakerFeeBasisPoints) external onlyAdmin {
    require(_tradeTakerFeeBasisPoints < maxTradeFeeBasisPoints, 'Excessive taker fee');
    emit TradeTakerFeeChanged(tradeTakerFeeBasisPoints, _tradeTakerFeeBasisPoints);
    tradeTakerFeeBasisPoints = _tradeTakerFeeBasisPoints;
  }

  /*** Accessors ***/

  function balanceOf(address wallet, address asset) external view returns (uint256 balance) {
  ...
  /**
    ...
    */
  function partiallyFilledOrderQuantity(uint128 orderId) external view returns (uint64 quantity) {
  ...
  function depositTokenBySymbol(string calldata tokenSymbol, uint256 quantity) external {
    address tokenAddress = tokens.tokenSymbolToAddress(tokenSymbol, uint64(block.timestamp * 1000));
    require(tokenAddress != address(0x0), 'Use depositEther to deposit Ether');
    deposit(msg.sender, tokenAddress, quantity);
  }

  function deposit(address payable wallet, address asset, uint256 quantity) private {
    ...
    if(asset != address(0x0)) {
      ...
      require(token.exists && token.isConfirmed, 'Token registration not confirmed');
    }
    ...
    uint256 tokenQuantityInPipPrecision = tokens.pipsToTokenQuantity(quantityInPips, asset);
    uint256 newBalance = balances[wallet][asset].add(tokenQuantityInPipPrecision);
  ...
  function invalidateOrderNonce(uint128 nonce) external {
    ...
    emit InvalidatedOrderNonce(msg.sender, nonce, timestamp, block.number + chainPropagationDelay);
  }
  ...
  function withdraw(
    ...
    require(
      getFeeBasisPoints(withdrawal.gasFee, withdrawal.quantity) <= withdrawalFeeBasisPoints,
      'Excessive withdrawal fee'
    );
    validateWithdrawalSignature(withdrawal, withdrawalTokenSymbol, withdrawalWalletSignature);
    ...
    address assetAddress = withdrawal.withdrawalType == WithdrawalType.BySymbol
      ? tokens.tokenSymbolToAddress(withdrawalTokenSymbol, getTimestampFromUuid(withdrawal.nonce))
      : withdrawal.assetAddress;

    // SafeMath reverts if overdrawn
    uint256 quantityInWei = tokens.pipsToTokenQuantity(withdrawal.quantity, assetAddress);
    balances[withdrawal.walletAddress][assetAddress] = balances[withdrawal
      .walletAddress][assetAddress]
      .sub(quantityInWei);
    balances[feeWallet][withdrawal.assetAddress] = balances[feeWallet][assetAddress].add(
    ...
    ICustodian(custodian).withdraw(
      ...
      assetAddress,
      tokens.pipsToTokenQuantity(withdrawal.quantity.sub(withdrawal.gasFee), assetAddress)
    );
  ...
  function exitWallet() external {
    ...
    walletExits[msg.sender] = WalletExit(true, block.number + chainPropagationDelay);
   ...
   */
  function updateBalancesForTrade(Order memory buy, Order memory sell, Trade memory _trade) private {
    // Buyer receives base asset minus fees
    balances[buy.walletAddress][buy.baseAssetAddress] = balances[buy.walletAddress][buy
      .baseAssetAddress]
      .add(tokens.pipsToTokenQuantity(_trade.netBaseQuantity, buy.baseAssetAddress));
    // Buyer gives quote asset including fees
    balances[buy.walletAddress][buy.quoteAssetAddress] = balances[buy.walletAddress][buy
      .quoteAssetAddress]
      .sub(tokens.pipsToTokenQuantity(_trade.grossQuoteQuantity, buy.quoteAssetAddress));

    // Seller gives base asset including fees
    balances[sell.walletAddress][sell.baseAssetAddress] = balances[sell.walletAddress][sell
      .baseAssetAddress]
      .sub(tokens.pipsToTokenQuantity(_trade.grossBaseQuantity, sell.baseAssetAddress));
    // Seller receives quote asset minus fees
    balances[sell.walletAddress][sell.quoteAssetAddress] = balances[sell.walletAddress][sell
      .quoteAssetAddress]
      .add(tokens.pipsToTokenQuantity(_trade.netQuoteQuantity, sell.quoteAssetAddress));

    // Maker and taker fees to fee wallet
    balances[feeWallet][_trade.makerFeeAssetAddress] = balances[feeWallet][_trade.makerFeeAssetAddress]
      .add(tokens.pipsToTokenQuantity(_trade.makerFeeQuantity, _trade.makerFeeAssetAddress));
    balances[feeWallet][_trade.takerFeeAssetAddress] = balances[feeWallet][_trade.takerFeeAssetAddress]
      .add(tokens.pipsToTokenQuantity(_trade.takerFeeQuantity, _trade.takerFeeAssetAddress));
  }

  function updateOrderFilledQuantities(Order memory buy, Order memory sell, Trade memory _trade)
    private
  {
   ...
   */
  function updateOrderFilledQuantity(Order memory order, Trade memory _trade) private {
  ...
  function validateAssetPair(
    ...
    validateTokenAddresses(sell, sellBaseSymbol, sellQuoteSymbol, OrderSide.Sell);

    // Both orders must be for same asset pair
    require(buy.baseAssetAddress == sell.baseAssetAddress, 'Base asset mismatch');
    require(buy.quoteAssetAddress == sell.quoteAssetAddress, 'Quote asset mismatch');
  ...
  function validateLimitPrices(Order memory buy, Order memory sell, Trade memory _trade) private pure {
    require(_trade.grossBaseQuantity > 0, 'Base amount must be greater than zero');
    require(_trade.grossQuoteQuantity > 0, 'Quote amount must be greater than zero');
    uint64 price = _trade.grossQuoteQuantity.mul(10**8).div(_trade.grossBaseQuantity);

    bool exceedsBuyLimit = buy.orderType == OrderType.Limit && price > buy.limitPrice;
    require(!exceedsBuyLimit, 'Buy order limit price exceeded');

    bool exceedsSellLimit = sell.orderType == OrderType.Limit && price < sell.limitPrice;
  ...
  function validateTokenAddresses(
    ...
    address baseAssetAddress = tokens.tokenSymbolToAddress(baseSymbol, timestamp);
    address quoteAssetAddress = tokens.tokenSymbolToAddress(quoteSymbol, timestamp);

    require(
      baseAssetAddress == order.baseAssetAddress && quoteAssetAddress == order.quoteAssetAddress,
      side == OrderSide.Buy
        ? 'Buy order market symbol address resolution mismatch'
        : 'Sell order market symbol address resolution mismatch'
    );
    require(
      baseAssetAddress == address(0x0) || tokens.tokensByAddress[baseAssetAddress].isConfirmed,
      side == OrderSide.Buy
        ? 'Buy order base asset registration not confirmed'
        : 'Sell order base asset registration not confirmed'
    );
    require(
      quoteAssetAddress == address(0x0) || tokens.tokensByAddress[quoteAssetAddress].isConfirmed,
      side == OrderSide.Buy
  ...
  function validateTradeFees(Order memory order, Trade memory _trade) private view {
    uint64 makerTotalQuantity = _trade.makerFeeAssetAddress == order.baseAssetAddress
      ? _trade.grossBaseQuantity
      : _trade.grossQuoteQuantity;
    uint64 takerTotalQuantity = _trade.takerFeeAssetAddress == order.baseAssetAddress
      ? _trade.grossBaseQuantity
      : _trade.grossQuoteQuantity;

    require(
      getFeeBasisPoints(_trade.makerFeeQuantity, makerTotalQuantity) <= tradeMakerFeeBasisPoints,
      'Excessive maker fee'
    );
    require(
      getFeeBasisPoints(_trade.takerFeeQuantity, takerTotalQuantity) <= tradeTakerFeeBasisPoints,
      'Excessive taker fee'
    );
  }
  ...
  function validateOrderSignatures(
    ...
    require(
      ECRecovery.isSignatureValid(
        getOrderWalletHash(buy, buyBaseSymbol, buyQuoteSymbol, buyClientOrderId),
        buyWalletSignature,
        buy.walletAddress
      ),
    ...
    require(
      ECRecovery.isSignatureValid(
        getOrderWalletHash(sell, sellBaseSymbol, sellQuoteSymbol, sellClientOrderId),
        sellWalletSignature,
        sell.walletAddress
      ),
  ...
  function validateOrderNonces(Order memory buy, Order memory sell) private view {
    require(
      getTimestampFromUuid(buy.nonce) > getLastInvalidatedTimestamp(buy.walletAddress),
      'Buy order nonce timestamp too low'
    );
    require(
      getTimestampFromUuid(sell.nonce) > getLastInvalidatedTimestamp(sell.walletAddress),
    ...
    returns (string memory)
  {
    ...
    bytes memory marketSymbolBytes = bytes(
      new string(baseSymbolBytes.length + quoteSymbolBytes.length + hyphenBytes.length)
    );
  ...
  function registerToken(address tokenAddress, string calldata symbol, uint8 decimals)
    external
    onlyAdmin
  {
  ...
  function confirmTokenRegistration(address tokenAddress, string calldata symbol, uint8 decimals)
    external
    onlyAdmin
  {
  ...
  function getTokenByAddress(address tokenAddress) external view returns (Tokens.Token memory) {
  ...
  function setDispatcher(address _dispatcher) external onlyAdmin {
    ...
    require(_dispatcher != dispatcher, 'Must be different from current dispatcher');
  ...
  function getFeeBasisPoints(uint64 fee, uint64 total) private pure returns (uint64) {
    return fee.mul(10000).div(total);
  }

  function getLastInvalidatedTimestamp(address walletAddress) private view returns (uint64) {
    if (
  ...
  function getTimestampFromUuid(uint128 uuid) private pure returns (uint64) {
    ...
    uint64 msSinceUnixEpoch = uint64(nsSinceGregorianEpoch / 10000) - 12219292800000;
